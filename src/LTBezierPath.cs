...
public class LTBezierPath
{
	...
	public void place2d(Transform transform, float ratio)
	{
		...
		if (ratio <= 1f)
		{
			float angle = Vector3.Angle(transform.position, this.point(ratio) - transform.position);
	...
	public void placeLocal2d(Transform transform, float ratio)
	{
		...
		if (ratio <= 1f)
		{
			float angle = Vector3.Angle(transform.localPosition, transform.parent.TransformPoint(this.point(ratio)) - transform.localPosition);
			transform.rotation = Quaternion.identity;
			transform.RotateAround(transform.position, Vector3.up, 90f);
			transform.RotateAround(transform.position, Vector3.right, angle);
		}
