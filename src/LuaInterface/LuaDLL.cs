...
namespace LuaInterface
{
	public class LuaDLL
	{
		...
		public static void lua_pushlstring(IntPtr luaState, byte[] str, int size)
		{
			if (size >= 2147483392)
			{
				throw new LuaException("string length overflow", null, 1);
			}
		...
		public static int luaL_typerror(IntPtr L, int stackPos, string tname, string t2 = null)
		{
			...
			string extramsg = string.Format("{0} expected, got {1}", tname, t2);
		...
		{
			int num = LuaDLL.tolua_tointeger(L, stackPos);
		...
		public static int luaL_throw(IntPtr L, string message)
		{
			...
			LuaDLL.lua_pushstring(L, message);
			LuaDLL.lua_pushnumber(L, 1.0);
			if (LuaDLL.lua_pcall(L, 2, -1, 0) == 0)
			{
				message = LuaDLL.lua_tostring(L, -1);
			}
			else
			{
				LuaDLL.lua_pop(L, 1);
		...
		{
			long num = LuaDLL.tolua_toint64(L, stackPos);
			if (num == 0L && !LuaDLL.tolua_isint64(L, stackPos))
			{
				LuaDLL.luaL_typerror(L, stackPos, "long", null);
				return 0L;
			}
			return num;
		}
		...
		{
			ulong num = LuaDLL.tolua_touint64(L, stackPos);
			if (num == 0uL && !LuaDLL.tolua_isuint64(L, stackPos))
			{
				LuaDLL.luaL_typerror(L, stackPos, "ulong", null);
				return 0uL;
			}
			return num;
		}
		...
		{
			IntPtr functionPointerForDelegate = Marshal.GetFunctionPointerForDelegate(func);
			LuaDLL.tolua_pushcfunction(luaState, functionPointerForDelegate);
		}
